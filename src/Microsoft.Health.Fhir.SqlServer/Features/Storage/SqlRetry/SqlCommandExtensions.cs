...
namespace Microsoft.Health.Fhir.SqlServer.Features.Storage
{
    ...
    {
        public static async Task ExecuteNonQueryAsync(this SqlCommand cmd, ISqlRetryService retryService, ILogger logger, CancellationToken cancellationToken, string logMessage = null, bool isReadOnly = false)
        {
        ...
        {
            object scalar = null;
            await retryService.ExecuteSql(cmd, async (sql, cancel) => { scalar = await sql.ExecuteScalarAsync(cancel); }, logger, logMessage, cancellationToken, isReadOnly);
            return scalar;
        }
    }
}
